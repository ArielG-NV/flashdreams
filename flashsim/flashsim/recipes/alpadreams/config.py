...
Mirrors the legacy ``projects/alpadreams/config.py`` presets:

- ``sv_2steps_chunk2_loc6_lightvae_lighttae``: single-view, len_t=2,
  light Wan VAE encoder + LightTAE decoder.
- ``sv_2steps_chunk2_loc6_vae_vae``: single-view, len_t=2, full Wan VAE
  for both encoder and decoder.
- ``sv_2steps_chunk3_loc6_vae_vae``: single-view, len_t=3, full Wan
  VAE for both.
- ``sv_2steps_chunk4_loc8_pshuffle_lighttae``: single-view, len_t=4,
  PixelShuffle pseudo-VAE for HDMap encoding + LightTAE decoder.
- ``mv_2steps_chunk4_loc8_pshuffle_lighttae``: 4-view, len_t=4,
  PixelShuffle + LightTAE.

Builder functions take only the runtime knobs the recipe layer must
own (``cp_size``, ``compile_network``, ``seed``).
"""
...
from flashsim.infra.diffusion.scheduler.fm import (
