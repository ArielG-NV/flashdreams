"""Flashsim-native VideoModelAPI adapter for gRPC serving."""
...
class FlashsimPipelineLatentCache(BaseLatentCache):
...
class FlashsimPipelineVideoModelAPI(VideoModelAPI[FlashsimPipelineLatentCache]):
    """VideoModelAPI backed by `AlpadreamsPipeline`."""

    latent_cache_type = FlashsimPipelineLatentCache
    ...
    def __init__(
        ...
        *,
        ...
        if num_frames_per_block % 4 != 0:
            raise ValueError(
                "num_frames_per_block must be divisible by 4 for flashsim pipeline backend"
            )

        if upsampler != "none":
            raise ValueError(
                "Upsampler support is not wired in flashsim pipeline backend yet."
            )
        if not use_cuda_graphs:
            logger.warning(
                "use_cuda_graphs flag is ignored by flashsim pipeline backend."
            )
        if kv_cache_on_side_stream:
            logger.warning(
                "kv_cache_on_side_stream flag is ignored by flashsim pipeline backend."
            )
        if s3_credential_path != "credentials/s3_checkpoint.secret":
            logger.warning(
                "s3_credential_path is controlled by flashsim checkpoint loader defaults."
            )
    ...
    def finalize_block_generation(
        self,
        latent_cache: FlashsimPipelineLatentCache,
    ...
    def start_generation(
        ...
        view_names: list[str],
    ) -> tuple[FlashsimPipelineLatentCache, Tensor, dict]:
