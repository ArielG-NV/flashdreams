...
def load_laneline_geometry_config() -> Dict:
    ...
    config_path = (
        ...
        / "color_scheme"
        / "config_color_geometry_laneline.json"
    )
...
def create_long_dashed_segments(
    line_segments: np.ndarray, segment_interval: float = 0.05
) -> np.ndarray:
    """
    ...
    while pattern_start < total_length:
        ...
        start_idx = np.searchsorted(
            cumulative_distances[1:], pattern_start, side="right"
        )
        ...
        if start_idx <= end_idx and end_idx < len(line_segments):
            ...
            for i in range(start_idx, end_idx + 1):
                if (
                    cumulative_distances[i] >= pattern_start
                    and cumulative_distances[i + 1] <= dash_end
                ):
                    dashed_segments.append(line_segments[i])
                elif (
                    cumulative_distances[i] < pattern_start
                    and cumulative_distances[i + 1] > pattern_start
                ):
                    # Partial segment at start
                    t = (pattern_start - cumulative_distances[i]) / segment_lengths[i]
                    start_point = line_segments[i, 0] + t * (
                        line_segments[i, 1] - line_segments[i, 0]
                    )
                    if cumulative_distances[i + 1] <= dash_end:
                        dashed_segments.append(
                            np.array([start_point, line_segments[i, 1]])
                        )
                    else:
                        t_end = (dash_end - cumulative_distances[i]) / segment_lengths[
                            i
                        ]
                        end_point = line_segments[i, 0] + t_end * (
                            line_segments[i, 1] - line_segments[i, 0]
                        )
                        dashed_segments.append(np.array([start_point, end_point]))
                elif (
                    cumulative_distances[i] < dash_end
                    and cumulative_distances[i + 1] > dash_end
                ):
                    # Partial segment at end
                    t = (dash_end - cumulative_distances[i]) / segment_lengths[i]
                    end_point = line_segments[i, 0] + t * (
                        line_segments[i, 1] - line_segments[i, 0]
                    )
...
def create_short_dashed_segments(
    line_segments: np.ndarray, segment_interval: float = 0.05
) -> np.ndarray:
    """
    ...
    while pattern_start < total_length:
        ...
        if start_idx <= end_idx and end_idx < len(line_segments):
            for i in range(start_idx, min(end_idx + 1, len(line_segments))):
                if (
                    cumulative_distances[i] >= pattern_start
                    and cumulative_distances[i + 1] <= dot_end
                ):
...
def create_dual_line_segments(
    line_segments: np.ndarray, separation: float = 0.15
...
def create_dot_dashed_segments(
    line_segments: np.ndarray, segment_interval: float = 0.05
) -> np.ndarray:
    """
...
def offset_line_segments(
    line_segments: np.ndarray, offset_distance: float = 0.1
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def prepare_laneline_geometry_data(
    lanelines_with_types: List[Tuple[np.ndarray, str]],
) -> List[Dict]:
    """
    ...
    for polyline, lane_type in lanelines_with_types:
        ...
        polyline_subdivided = interpolate_polyline_to_points(
            polyline, segment_interval=0.05
        )
        ...
        line_segments = np.stack(
            [polyline_subdivided[:-1], polyline_subdivided[1:]], axis=1
        )
